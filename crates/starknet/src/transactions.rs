...
impl StarknetTransaction {
    ...
    pub fn get_l2_to_l1_messages(&self) -> Vec<MessageToL1> {
        ...
        fn get_blockifier_messages_recursively(call_info: &CallInfo, caller_address: ContractAddress) -> Vec<OrderedMessageToL1> {
            let mut messages = vec![];
            ...
            messages.extend(call_info.execution.l2_to_l1_messages.iter().map(|m| {
                OrderedMessageToL1 {
                    ...
                    message: MessageToL1 {
                        ...
                        from_address: caller_address.into(),
            ...
            call_info.inner_calls.iter().for_each(|call| {
                messages.extend(get_blockifier_messages_recursively(call,  call.call.storage_address.into()));
            });
        ...
        for inner_call_info in call_infos {
            let mut not_sorted_messages = get_blockifier_messages_recursively(inner_call_info, inner_call_info.call.storage_address.into());
